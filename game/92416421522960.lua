...
local S = {
    ...
    CombatEnabled = false, CombatMethod = "Teleport", CombatSpeed = 1, CombatTweenDistance = 10,
    ...
    DiceEnabled = false, DiceSaveRolls = {}, DiceSelection = {}, DiceUseOnUnpause = false,
    _diceStackBusy = false, _diceStackPhase = 0,
...
do
    ...
    Tabs.Upgrade:Space({ Columns = 0.5 })
    ...
    while task.wait() do
        if S.DiceEnabled and #S.DiceSaveRolls > 0 then
            pcall(function()
                ...
                -- Step 1: Priority-based pause (Only pause a roll if ALL higher-priority rolls are already paused at 0)
                local allHigherPaused = true
                ...
                for i, kind in ipairs(ordered) do
                    ...
                    if rollsLeft <= 0 then
                        -- This roll is at 0, pause it if not already
                        if not isPaused and allHigherPaused then
                            pauseRoll(kind, true)
                        end
                        currentPhase = i
                    else
                        ...
                        allAtZero = false
                        if allHigherPaused then
                            -- This is the current phase we're waiting on
                            currentPhase = i
                        end
                        -- Don't pause this one yet (higher priority not done)
                        break
                    end
                    
                    -- Check if this one is paused for next iteration
                    if not isPaused and rollsLeft > 0 then
                        allHigherPaused = false
                    end
                end
                ...
                if allAtZero then
                    ...
                    -- Arm dice item (if enabled, priority order)
                    if S.DiceUseOnUnpause and #S.DiceSelection > 0 then
                        for _, dice in ipairs(S.DiceSelection) do
                            local owned = items[dice] or 0
                            if owned > 0 then
                                netFetch("InventoryService", "requestUseItem", dice)
                                task.wait(0.3) -- Wait for arm to register
                                break -- Priority: only use first available
                            end
                        end
                    end
                    
                    -- Unpause ALL at once (rapid fire, no delay between)
                    ...
                    -- Resume auto-roll (next roll will trigger all specials + dice)
